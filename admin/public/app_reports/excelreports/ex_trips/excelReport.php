...
*/
 ...
 class excelReport
 {
    ...
    'underline' => PHPExcel_Style_Font::UNDERLINE_DOUBLE
			...
			$this->objExcel->getActiveSheet()->setCellValue('B1','COMPANYS ALL TRIPS REPORT');
	  ...
	  $this->objExcel->getActiveSheet()->SetCellValue('B5', 'Requestor Name');
	  $this->objExcel->getActiveSheet()->SetCellValue('C5', 'Requestor\'s Phone');
	  $this->objExcel->getActiveSheet()->SetCellValue('D5', 'Rider Name');
	  $this->objExcel->getActiveSheet()->SetCellValue('E5', 'Rider\'s Phone');
	  $this->objExcel->getActiveSheet()->SetCellValue('F5', 'Item Type');
	  $this->objExcel->getActiveSheet()->SetCellValue('G5', 'Quantity');
	  $this->objExcel->getActiveSheet()->SetCellValue('H5', 'Origin (From)');
	  $this->objExcel->getActiveSheet()->SetCellValue('I4', 'Destination (To)');
	  $this->objExcel->getActiveSheet()->SetCellValue('J4', 'Pickup Date & Time');
	  $this->objExcel->getActiveSheet()->SetCellValue('K4', 'Total (GHC)');
	...
	// 	//    var_dump($value['RES_STAFFID']); die();		
			...
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("1",$i,$value['REQ_REQUESTORNAME']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("2",$i,$value['REQ_REQUESTOR_PHONE']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("3",$i,$value['REQ_RIDERNAME']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("4",$i,$value['REQ_RIDERPHONE']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("5",$i,$value['REQ_ITEMS']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("6",$i,$value['REQ_TOTAL_ITEMS']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("7",$i,$value['REQ_LOCATION_FROM']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("8",$i,$value['REQ_LOCATIONSS']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("9",$i,$value['REQ_PICKUP_DATE'].' '.$value['REQ_PICKUP_TIME']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("10",$i,$value['REQ_TOTAL_AMOUNT']);
...
$this->objExcel->getActiveSheet()->setPageMargins($this->PageMargins);
     ...
     // worksheet name
     $this->objExcel->getActiveSheet()->setTitle('COMPANYS All TRIPS REPORT'); 
	 
	//Output to client browser
	  	// Redirect output to a client's web browser (Excel2007)
	  
	  	header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
	  	header('Content-Disposition: attachment;filename="'."COMPANYS_ALLTRIPS_REPORT".'.xls"');
