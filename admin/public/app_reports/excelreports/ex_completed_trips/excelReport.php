...
*/
 ...
 class excelReport
 {
    ...
    'underline' => PHPExcel_Style_Font::UNDERLINE_DOUBLE
			...
			$this->objExcel->getActiveSheet()->setCellValue('B1','COMPANYS COMPLETED TRIPS REPORT');
	  ...
	  $this->objExcel->getActiveSheet()->SetCellValue('B5', 'Created By');
	  $this->objExcel->getActiveSheet()->SetCellValue('C5', 'Category');
	  $this->objExcel->getActiveSheet()->SetCellValue('D5', 'Total Amount (GHS)');
	  $this->objExcel->getActiveSheet()->SetCellValue('E5', 'Amount Paid (GHS)');
	  $this->objExcel->getActiveSheet()->SetCellValue('F5', 'Unpaid Amount (GHS)');
	  $this->objExcel->getActiveSheet()->SetCellValue('G5', 'Date Added');
	...
	// 	//    var_dump($value['RES_STAFFID']); die();		
			...
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("1",$i,$value['EXPS_ACTORNAME']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("2",$i,$value['EXPS_CATNAME']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("3",$i,$value['EXPS_TOTALAMOUNT']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("4",$i,$value['EXPS_AMOUNTPAID']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("5",$i,(float)$value['EXPS_TOTALAMOUNT'] - (float)$value['EXPS_AMOUNTPAID']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("6",$i,$value['EXPS_DATEADDED']);
...
$this->objExcel->getActiveSheet()->setPageMargins($this->PageMargins);
     ...
     // worksheet name
     $this->objExcel->getActiveSheet()->setTitle('COMPANYS COMPLETED TRIPS REPORT'); 
	 
	//Output to client browser
	  	// Redirect output to a client's web browser (Excel2007)
	  
	  	header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
	  	header('Content-Disposition: attachment;filename="'."COMPANYS_COMPLETEDTRIPS_REPORT".'.xls"');
