...
*/
 ...
 class excelReport
 {
    ...
    'underline' => PHPExcel_Style_Font::UNDERLINE_DOUBLE
			...
			$this->objExcel->getActiveSheet()->setCellValue('B1','COMPANY\'S PROFIT & LOSS REPORT');
	  ...
	  $this->objExcel->getActiveSheet()->SetCellValue('B5', 'Transaction Code');
	  $this->objExcel->getActiveSheet()->SetCellValue('C5', 'Transaction Date');
	  $this->objExcel->getActiveSheet()->SetCellValue('D5', 'Amount');
	  $this->objExcel->getActiveSheet()->SetCellValue('E5', 'Paytype');
	  $this->objExcel->getActiveSheet()->SetCellValue('F5', 'Payment Mode');
	  $this->objExcel->getActiveSheet()->SetCellValue('G5', 'Rider\'s Name');
	  $this->objExcel->getActiveSheet()->SetCellValue('H5', 'Date Added');
	//   $this->objExcel->getActiveSheet()->SetCellValue('I5', 'Total Profit (GHS)');
	//   $this->objExcel->getActiveSheet()->SetCellValue('J5', 'Total Amount (GHS)');
	//   $this->objExcel->getActiveSheet()->SetCellValue('K5', 'Nationality');
	//   $this->objExcel->getActiveSheet()->SetCellValue('L5', 'ID Type');
	//   $this->objExcel->getActiveSheet()->SetCellValue('M5', 'ID Number');
	//   $this->objExcel->getActiveSheet()->SetCellValue('N5', 'Ghana Post GPS');
	//   $this->objExcel->getActiveSheet()->SetCellValue('O5', 'Gender');
	//   $this->objExcel->getActiveSheet()->SetCellValue('P5', 'External Account');
	//   $this->objExcel->getActiveSheet()->SetCellValue('Q5', 'Employer');
	//   $this->objExcel->getActiveSheet()->SetCellValue('R5', 'Address');
	...
	// 	//    var_dump($value['RES_STAFFID']); die();		
			...
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("1",$i,$value['PFT_TRANSCODE']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("2",$i,$value['PFT_TRANDATE']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("3",$i,$value['PFT_AMOUNT']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("4",$i,$value['PFT_PAYTYPE']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("5",$i,$value['PFT_PAYMODE']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("6",$i,$value['PFT_RIDERNAME']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("7",$i,$value['PFT_DATEADDED']);
			// $this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("8",$i,$value['SALE_PROFIT_TOTAL']);
			// $this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("9",$i,$value['SALE_GRANDTOTAL']);
...
$this->objExcel->getActiveSheet()->setPageMargins($this->PageMargins);
     ...
     // worksheet name
     $this->objExcel->getActiveSheet()->setTitle('COMPANY\'S PROFIT-LOSS REPORT'); 
	 
	//Output to client browser
	  	// Redirect output to a client's web browser (Excel2007)
	  
	  	header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
	  	header('Content-Disposition: attachment;filename="'."COMPANYS_PROFITLOSS_REPORT".'.xls"');
