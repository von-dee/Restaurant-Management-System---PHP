...
*/
 ...
 class excelReport
 {
    ...
    'underline' => PHPExcel_Style_Font::UNDERLINE_DOUBLE
			...
			$this->objExcel->getActiveSheet()->setCellValue('B1','COMPANYS RIDERS REPORT');
	  ...
	  $this->objExcel->getActiveSheet()->SetCellValue('C5', 'Date of Birth');
	  $this->objExcel->getActiveSheet()->SetCellValue('D5', 'Home Address');
	  $this->objExcel->getActiveSheet()->SetCellValue('E5', 'Phone Number');
	  $this->objExcel->getActiveSheet()->SetCellValue('F5', 'Email');
	  $this->objExcel->getActiveSheet()->SetCellValue('G5', 'Bike Name');
	  $this->objExcel->getActiveSheet()->SetCellValue('H5', 'Bike Number Plate');
	  $this->objExcel->getActiveSheet()->SetCellValue('I4', 'Date Added');
	...
	// 	//    var_dump($value['RES_STAFFID']); die();		
			...
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("1",$i,$value['RDR_FIRSTNAME'].' '.$value['RDR_LASTNAME']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("2",$i,$value['RDR_DATEOFBIRTH']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("3",$i,$value['RDR_HOMEADDRESS']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("4",$i,$value['RDR_PHONE']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("5",$i,$value['RDR_EMAIL']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("6",$i,$value['RDR_BIKE_NAME']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("7",$i,$value['RDR_BIKE_NUMBERPLATE']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("8",$i,$value['RDR_DATEADDED']);
...
$this->objExcel->getActiveSheet()->setPageMargins($this->PageMargins);
     ...
     // worksheet name
     $this->objExcel->getActiveSheet()->setTitle('COMPANYS RIDERS REPORT'); 
	 
	//Output to client browser
	  	// Redirect output to a client's web browser (Excel2007)
	  
	  	header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
	  	header('Content-Disposition: attachment;filename="'."COMPANYS_RIDERS_REPORT".'.xls"');
