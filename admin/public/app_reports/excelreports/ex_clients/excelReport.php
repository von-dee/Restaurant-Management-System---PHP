...
*/
 ...
 class excelReport
 {
    ...
    'underline' => PHPExcel_Style_Font::UNDERLINE_DOUBLE
			...
			$this->objExcel->getActiveSheet()->setCellValue('B1','COMPANY\'S CLIENTS REPORT');
	  ...
	  $this->objExcel->getActiveSheet()->SetCellValue('B5', 'Full Name');
	  $this->objExcel->getActiveSheet()->SetCellValue('C5', 'Phone Number');
	  $this->objExcel->getActiveSheet()->SetCellValue('D5', 'Email');
	  $this->objExcel->getActiveSheet()->SetCellValue('E5', 'User Type');
	  $this->objExcel->getActiveSheet()->SetCellValue('F5', 'Date Added');
	//   $this->objExcel->getActiveSheet()->SetCellValue('G5', 'Change (GHS)');
	//   $this->objExcel->getActiveSheet()->SetCellValue('H5', 'Total Cost (GHS)');
	//   $this->objExcel->getActiveSheet()->SetCellValue('I5', 'Total Loss (GHS)');
	...
	// 	//    var_dump($value['RES_STAFFID']); die();		
			...
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("1",$i,$value['USR_FIRSTNAME'].' '.$value['USR_OTHERNAME']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("2",$i,$value['USR_PHONE']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("3",$i,$value['USR_EMAIL']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("4",$i,$value['USR_ACCESS_LEVEL']);
			$this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("5",$i,$value['USR_DATE_ADDED']);
			// $this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("6",$i,$value['SALE_CHANGE']);
			// $this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("7",$i,$value['SALE_COST_TOTAL']);
			// $this->objExcel->getActiveSheet()->SetCellValueByColumnAndRow("8",$i,$value['SALE_LOSS_TOTAL']);
...
$this->objExcel->getActiveSheet()->setPageMargins($this->PageMargins);
     ...
     // worksheet name
     $this->objExcel->getActiveSheet()->setTitle('COMPANY\'S CLIENTS REPORT'); 
	 
	//Output to client browser
	  	// Redirect output to a client's web browser (Excel2007)
	  
	  	header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
	  	header('Content-Disposition: attachment;filename="'."COMPANYS_CLIENTS_REPORT".'.xls"');
